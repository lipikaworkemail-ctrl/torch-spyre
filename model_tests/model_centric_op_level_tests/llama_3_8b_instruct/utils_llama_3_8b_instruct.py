...
"""
Self-contained test utilities for Llama-3.1-8B-Instruct operator

Architecture values are sourced directly from the Hugging Face config.json for the Llama 3.1 model:
...
"head_dim": 160,           # derived = hidden_size // num_heads (5120 / 32)
"num_hidden_layers": 40,
"sliding_window": null,    # not used in Llama (kept for structural consistency)
...
Public API (consumed by test_llama_3_8b_instruct.py)
----------------------------------------------------------------------
...
# --- Dimensions ---
HIDDEN_SIZE = 4096        # Residual-stream / hidden dimension.
INTERMEDIATE_SIZE = 14336  # FFN gate/up/down intermediate dimension.
EMBED_DIM = 4096       # Embedding / projection dimension (= 40 * 128).
NUM_ATTENTION_HEADS = 32  #is NOT used (avoids pytest deselection).
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Model architecture constants (Llama-3.1-8B-Instruct)
# ─────────────────────────────────────────────────────────────────────────────
...
INTERMEDIATE_SIZE = 14336  # FFN gate/up/down intermediate dimension.
EMBED_DIM = 4096       # Embedding / projection dimension (= 40 * 128).
NUM_ATTENTION_HEADS = 32  # Used for weight-shape arithmetic only.
...
DECODE_PARAMS: dict = {
    ...
    for b, kv in [
        (1, 65), (1, 128), (1, 256), (1, 512), (1, 1024), (1, 2048),
        (2, 65), (2, 128),
        (4, 65), (4, 128),
        (8, 65),
    ]
...
DTYPE_PARAMS: dict = {
    "fp16": make_qkv(1, 64, 64, dtype=torch.float16,    diff="fp16"),
    "bf16": make_qkv(1, 64, 64, dtype=torch.bfloat16,   diff="bf16"),
    "fp32": make_qkv(1, 64, 64, dtype=torch.float32,    diff="fp32"),
}

NUMERIC_COVERAGE_PARAMS: dict = {
    **{
        f"seed{i:02d}_seq64": make_qkv(1, 64, 64, diff=("cov", i, "seq64"))
...
GROWING_KV_PARAMS: dict = {
    ...
    for kv in [1, 2, 4, 8, 16, 32, 64, 65, 128]
}
